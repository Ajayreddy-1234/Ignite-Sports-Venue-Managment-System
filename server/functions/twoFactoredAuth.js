...
async function twoFactoredMail({email}){
    try{
        const [result] = await db.promise().query('SELECT * FROM ignite.User WHERE email = ? ',[email]);
        if(result.length == 0){
           throw new Error("No user Found with this email");
        }
        const temp = Math.floor(100000 + Math.random() * 900000);
        const twoFactorOtp = temp.toString();
        await db.promise().query('update ignite.User SET two_factor_secret = ? where user_id = ?',[twoFactorOtp, result[0].user_id]);
        const mailOptions = {
