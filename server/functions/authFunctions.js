...
async function registerUser({ username, email, password, role }) {
    ...
    const [existingUser] = await db.promise().query('SELECT * FROM ignite.User WHERE username = ? OR email = ?', [username, email]);
...
async function loginUser({ username, password }) {
  try {
    const [users] = await db.promise().query('SELECT * FROM ignite.User WHERE username = ?', [username]);
    ...
    const [res] = await db.promise().query('SELECT * FROM ignite.Tokens WHERE userId = ?',[user.user_id]);
