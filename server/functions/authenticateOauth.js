...
async function oauthTokenize({email}){
  var [existingUser] = await db.promise().query('SELECT * FROM ignite.User WHERE email = ?', [email]);
  const token = jwt.sign({ userId: existingUser[0].id, username: existingUser[0].username }, process.env.ENV_SECRET_KEY, { expiresIn: '15m' });
  const [result] = await db.promise().query('SELECT * FROM ignite.Tokens WHERE userId = ?',[existingUser[0].user_id]);
  if (result.length == 0){
      await db.promise().query('INSERT INTO ignite.Tokens (userId, tokenId) VALUES ( ?, ?)',[existingUser[0].user_id,token]);
  }else{
